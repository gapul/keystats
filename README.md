...
- 入力速度は連続打鍵の間隔(gap<2秒を「フロー中」)から平均/ピークKPMと実打鍵時間を算出。
  Delete/Backspace は速度計測から除外(修正打鍵で速く見えるのを防ぐ)。修正率は削除打鍵/総打鍵。
- SQLite に集約: `counts / combos / apptime / kbtype / typing`(いずれも hour 粒度で UPSERT)。外部依存なし。
...
起動時にシステム言語を自動判定。メニューバー →「言語 / Language」から手動でも切替可能
(UserDefaults に保存)。CLI は `LANG` などのロケール、または `KEYSTATS_LANG=en|ja` で指定。
